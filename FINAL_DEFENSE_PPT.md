...
│ ────────────────────────────────────────────────────────────────────── │
│                                                                        │
│              IoT-ENABLED AUTOMATIC RECHARGE SYSTEM                     │
│              FOR KENYAN PREPAID METERS BASED ON                        │
│              ADVANCED METERING INFRASTRUCTURE                          │
│                                                                        │
│ ────────────────────────────────────────────────────────────────────── │
│                                                                        │
│   Registration No.          Name                      Signature        │
│   E021-01-1297/2021        MOSES MAINGI              ___________       │
│   E021-01-1302/2021        KELVIN LIMO               ___________       │
│   E021-01-1318/2021        DANIEL KIMOI              ___________       │
│                                                                        │
...
• **Over 8 million prepaid meters** deployed by Kenya Power (KPLC)

• **M-Pesa dominates payments** - 75%+ of electricity purchases via mobile money

• **Current recharge process is outdated:**
  - User pays via M-Pesa → Receives 20-digit SMS token → Must manually enter token into meter

• **Major pain points affecting millions:**
...
• **Economic impact:** Kenya Power loses revenue due to customer frustration and service delays
...
• Kenya's prepaid electricity system still relies on **manual 20-digit token entry** despite widespread M-Pesa adoption

• **SMS token delivery is unreliable** - network congestion causes delays/failures

• **CIU failures** leave customers unable to load purchased tokens

• **Existing smart meters are expensive** (KES 15,000-25,000) and lack M-Pesa integration

• **No automated end-to-end solution** exists that directly links M-Pesa payment to meter balance update

### Justification

• **3+ million KPLC customers** experience these issues daily

• **M-Pesa infrastructure is mature** (32M+ users) but underutilized for full automation

• **IoT technology costs have dropped** - ESP32 microcontroller costs <KES 800

• **Eliminates human intervention** - reduces errors, improves user experience

• **Cost-effective solution** using affordable, locally available components
...
│                     EXISTING SYSTEMS                                     │
│                                                                          │
│   [User Pays] → [Token Generated] → [SMS Sent] → [USER ENTERS TOKEN]    │
│                                                    ↑                     │
│                                              MANUAL STEP                 │
│                                              (Error-prone)               │
└─────────────────────────────────────────────────────────────────────────┘
...
│                       OUR SYSTEM                                         │
│                                                                          │
│   [User Pays] → [Backend Processes] → [MQTT Command] → [Meter Updated]  │
│                                                                          │
...
|-----|-------------------|
| **1. No M-Pesa automation** | Direct Daraja API → Meter balance update |
...
└─────────────────────────────────────────────────────────────────────────────────┘
                    ...
                    ▼                  ▼                  ▼
           ┌───────────────┐  ┌───────────────┐  ┌───────────────┐
           │  WEB DASHBOARD │  │   M-PESA APP  │  │  USSD (*334#) │
           │   (React.js)   │  │  (Safaricom)  │  │  Basic Phone  │
           └───────┬───────┘  └───────┬───────┘  └───────────────┘
                   │                  │
                   │ HTTPS            │ STK Push
                   ▼                  ▼
           ...
           └──────────────────┼──────────────────────┘
              ...
              ▼                             ▼
     ┌─────────────────┐           ┌─────────────────┐
     │ FIREBASE RTDB   │           │   ESP32 SMART   │
     │  (Database)     │           │     METER       │
     │                 │           │                 │
     │ • User Data     │◄─────────►│ • Wi-Fi Module  │
     │ • Balances      │   Sync    │ • SIM800L GSM   │
     │ • Transactions  │           │ • OLED Display  │
     │ • Consumption   │           │ • Status LEDs   │
     └─────────────────┘           │ • Load Control  │
                                   └────────┬────────┘
                                            │
                                            ▼
...
│  ─────────────────────────────────────────────────────────────────────────────── │
│  • User clicks "Recharge" on dashboard OR dials USSD                            │
│  • Enters amount (e.g., KES 100)                                                │
...
└─────────────────────────────────────────────────────────────────────────────────┘
                                       ...
                                       ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│  STEP 2: M-PESA PROCESSES PAYMENT                                                │
│  ─────────────────────────────────────────────────────────────────────────────── │
│  • User receives M-Pesa prompt on phone                                         │
│  • User enters M-Pesa PIN to confirm                                            │
│  • Safaricom processes transaction                                              │
│  • Time: 2-5 seconds (user-dependent)                                           │
└─────────────────────────────────────────────────────────────────────────────────┘
                                       │
                                       ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│  STEP 3: CALLBACK & DATABASE UPDATE                                              │
│  ─────────────────────────────────────────────────────────────────────────────── │
│  • Safaricom sends callback to our backend                                      │
│  • Backend verifies payment & updates Firebase                                  │
│  • Transaction logged with receipt number                                       │
│  • Time: ~300ms                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
                                       │
                                       ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│  STEP 4: MQTT COMMAND TO METER                                                   │
│  ─────────────────────────────────────────────────────────────────────────────── │
│  • Backend publishes balance update via MQTT                                    │
│  • Topic: smartmeter/{meterNo}/command/balance                                  │
│  • ESP32 receives command instantly                                             │
│  • Time: ~200ms                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
                                       │
                                       ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│  STEP 5: METER UPDATES & LOAD RECONNECTS                                         │
│  ─────────────────────────────────────────────────────────────────────────────── │
│  • ESP32 updates local balance                                                  │
│  • OLED display shows new balance                                               │
│  • If balance was 0, load automatically reconnects (LED turns ON)               │
│  • Confirmation buzzer sounds                                                   │
│  • SMS alert sent to user (optional)                                            │
│  • Time: ~100ms                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
...
```
    ...
    │   ┌─────────────┐     ┌─────────────┐     ┌─────────────┐      │
    │   │   ESP32     │     │    OLED     │     │   SIM800L   │      │
    │   │  DevKit V1  │     │  128x64     │     │  GSM/GPRS   │      │
    │   │             │     │  Display    │     │   Module    │      │
    │   │  • Dual Core│     │             │     │             │      │
    │   │  • Wi-Fi    │────►│  Shows:     │     │  • 2G Data  │      │
    │   │  • 520KB RAM│     │  • Balance  │     │  • SMS Alerts│     │
    │   │  • 4MB Flash│     │  • Status   │     │  • Fallback │      │
    │   └─────────────┘     └─────────────┘     └─────────────┘      │
    │          │                                       │              │
    │          │            ┌─────────────┐           │              │
    │          │            │   STATUS    │           │              │
...
│  │   └── Callback handler (payment confirmation)                                │
│  ├── MQTT Service                                                               │
...
└─────────────────────────────────────────────────────────────────────────────────┘
                                       ...
                                       ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│  TIER 3: DEVICE (ESP32 Firmware)                                                 │
│  ─────────────────────────────────────────────────────────────────────────────── │
│                                                                                  │
│  Technology: C++ (Arduino Framework)                                            │
│                                                                                  │
│  Modules:                                                                        │
│  ├── Wi-Fi Manager (connection, reconnection)                                   │
│  ├── MQTT Client (subscribe/publish)                                            │
│  ├── GSM Handler (SMS alerts, data fallback)                                    │
│  ├── Display Controller (OLED updates)                                          │
│  ├── Balance Manager (consumption simulation)                                   │
...
└─────────────────────────────────────────────────────────────────────────────────┘
              ...
              ▼                     ▼                     ▼
    ┌─────────────────┐   ┌─────────────────┐   ┌─────────────────┐
    │    BACKEND      │   │    ESP32        │   │   DASHBOARD     │
    │   PUBLISHES:    │   │   PUBLISHES:    │   │   SUBSCRIBES:   │
    │                 │   │                 │   │                 │
    │ smartmeter/     │   │ smartmeter/     │   │ (via Firebase)  │
    │ {meterNo}/      │   │ {meterNo}/      │   │                 │
    │ command/balance │   │ consumption     │   │                 │
    │                 │   │ status          │   │                 │
    │   SUBSCRIBES:   │   │ balance         │   │                 │
    │ smartmeter/+/   │   │                 │   │                 │
    │ consumption     │   │   SUBSCRIBES:   │   │                 │
    │ status          │   │ smartmeter/     │   │                 │
    │ balance         │   │ {meterNo}/      │   │                 │
    │                 │   │ command/balance │   │                 │
    └─────────────────┘   └─────────────────┘   └─────────────────┘
...
|-----------|---------------|---------------------|--------|
| ESP32 Wi-Fi | 802.11 b/g/n | Consistent @ 2.4GHz | ✅ Excellent |
| OLED Display | 128x64 pixels | Clear, high-contrast | ✅ Excellent |
| LED Load Simulation | 5mm Blue LED | Clear visual indication | ✅ Excellent |
| SIM800L GSM | 2G GPRS | SMS alerts working (Airtel) | ✅ Good |
| Power Consumption | Est. <500mA | Measured 280-320mA | ✅ Better |
...
│                    50 TEST CYCLES                               │
├────────────────────────────────────────────────────────────────┤
│  Disconnection Accuracy (balance = 0):     100% (50/50)        │
│  Reconnection Reliability (after recharge): 100% (50/50)       │
│  Average Reconnection Time:                 1-3 seconds        │
└────────────────────────────────────────────────────────────────┘
```
...
|--------|--------|----------|--------|
| Total Transaction Time | < 10s | **3.2s avg** | ✅ Exceeded |
| Automated Processing Time | < 5s | **1.8s avg** | ✅ Exceeded |
...
└────────────────────────────────────────────────────────────────────────────────┘
    ...
    Basic Smart Meter    ████████████████████  KES 8,000 - 12,000
...
│                                                                                  │
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐                  │
│  │   BALANCE       │  │   CONSUMPTION   │  │   RECHARGE      │                  │
│  │   ━━━━━━━━━     │  │   ━━━━━━━━━     │  │   ━━━━━━━━━     │                  │
│  │                 │  │                 │  │                 │                  │
│  │   KES 347.50    │  │   📊 Graph      │  │   [Recharge]    │                  │
│  │   ● Balance OK  │  │   Daily/Weekly  │  │   Quick amounts │                  │
│  │   Est: 28 hrs   │  │   Monthly view  │  │   50|100|200    │                  │
│  │                 │  │                 │  │                 │                  │
│  └─────────────────┘  └─────────────────┘  └─────────────────┘                  │
│                                                                                  │
└─────────────────────────────────────────────────────────────────────────────────┘
...
|-----------|--------|----------|
| 1. ESP32 Smart Meter Prototype | ✅ **ACHIEVED** | Working hardware with all features |
| 2. Cloud Backend + M-Pesa Integration | ✅ **ACHIEVED** | 48/50 successful sandbox tests |
| 3. User Dashboard Development | ✅ **ACHIEVED** | 4.8/5 user satisfaction rating |
| 4. MQTT Communication | ✅ **ACHIEVED** | 99.7% message success rate |
| 5. System Validation | ✅ **ACHIEVED** | 100% transaction accuracy |
...
✅ **Technological:** Proves affordable AMI is achievable for developing economies  
✅ **Economic:** KES 10.7M annual savings potential per 10,000 meters  
✅ **Social:** Addresses pain points affecting 3M+ Kenyan prepaid customers  
...
└─────────────────────────────────────────────────────────────────────────────────┘
    ...
    │                                                                │
    │   ┌─────────────┐        ┌─────────────┐        ┌───────────┐ │
    │   │   ACTIVE    │◄──────►│ LOW_BALANCE │◄──────►│DISCONNECTED│ │
    │   │             │        │             │        │            │ │
    │   │ 🟢 Green ON │        │ 🔴 Blinking │        │ 🔴 Solid   │ │
    │   │ 🔵 Load ON  │        │ 🔵 Load ON  │        │ 🔵 Load OFF│ │
    │   │ Balance > 50│        │ Balance<50  │        │ Balance = 0│ │
    │   └─────────────┘        └─────────────┘        └───────────┘ │
    │         ▲                                              │       │
    │         │                   RECHARGE                   │       │
    │         └──────────────────────────────────────────────┘       │
...
- "What about security?" → TLS planned for production
- "Why 2G GSM?" → Budget constraint, proves concept, upgrade path clear
- "What if WiFi fails?" → GSM fallback, queued commands
- "Real relay vs LED?" → Safety for development, same GPIO logic
