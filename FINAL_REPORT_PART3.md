...
**Objective 1: Design and Prototype an ESP32-Based Smart Meter Simulation System**

To design and construct a functional prototype of a prepaid electricity meter using the ESP32 microcontroller platform, incorporating essential components including:
- Real-time balance monitoring and display via OLED screen
- Automated load control simulation using LED indicators
- Visual (multi-LED) and audible alert systems
- Hybrid connectivity via Wi-Fi primary and GSM fallback
- Bi-directional communication capabilities via MQTT protocol
- Simulated consumption logic with configurable usage rates

This prototype serves as a proof-of-concept demonstrating that affordable, commercially available IoT hardware can replicate and enhance the functionality of traditional prepaid meters while enabling advanced features.

**Objective 2: Develop and Integrate Cloud-Based Backend with M-Pesa Daraja API**

To architect and implement a scalable cloud backend system that:
- Processes M-Pesa payment callbacks from Daraja API in real-time
- Manages user accounts and meter-to-user mapping in Firebase Realtime Database
- Implements secure RESTful API endpoints for system integration
- Generates and transmits automated recharge commands to ESP32 meters without manual intervention
- Maintains comprehensive transaction logs and audit trails
- Provides HTTPS encryption and authentication mechanisms

This backend infrastructure demonstrates how cloud computing and mobile payment platforms can be integrated to create a seamless, automated utility payment and service delivery system.
...
- Access detailed transaction history and payment records
- Initiate recharges directly through M-Pesa integration
- Visualize consumption patterns through interactive charts and graphs
- Receive notifications for low balance and successful recharges
- Manage account settings and preferences

This dashboard transforms raw system data into actionable insights, empowering consumers with unprecedented visibility and control over their electricity usage.

**Objective 4: Validate System Functionality and Performance Through Comprehensive Testing**

To conduct rigorous testing protocols including:
- **Integration Testing:** Verify seamless communication between ESP32, cloud backend, Firebase database, and M-Pesa API simulation across the entire transaction lifecycle
- **Performance Testing:** Measure and analyze transaction processing latency, system response times, network communication efficiency, and database query performance
- **Reliability Testing:** Assess system uptime, fault tolerance, and recovery mechanisms under various failure scenarios (network interruptions, database unavailability, concurrent user loads)
- **Security Testing:** Validate encryption protocols (HTTPS/TLS), authentication mechanisms, and data privacy safeguards
- **Functional Testing:** Confirm accurate balance updates, precise load control operations (disconnection/reconnection), and correct alert triggering

This comprehensive testing demonstrates the system's readiness for real-world deployment and identifies any remaining limitations or areas for optimization.
