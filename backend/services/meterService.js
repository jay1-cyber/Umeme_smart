...
async function processPayment(userId, meterNo, amount, reference) {
  ...
  // Send balance update to ESP32 via MQTT
  try {
    mqttService.sendBalanceToESP32(meterNo, newBalance);
    console.log(`[MQTT] Sent balance update to ESP32: Meter ${meterNo}, Balance: ${newBalance}`);
  } catch (error) {
    console.error('[MQTT] Failed to send balance update:', error.message);
  }
